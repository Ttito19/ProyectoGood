...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_cargo` (IN `id` INT, IN `nom` VARCHAR(100))  NO SQL
UPDATE cargo SET nombre=nom WHERE idcargo=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_cliente` (IN `idcli` INT, IN `nom` VARCHAR(100), IN `ape` VARCHAR(100), IN `co` VARCHAR(100), IN `nomde` VARCHAR(100), IN `nompro` VARCHAR(100), IN `nomdi` VARCHAR(100), IN `dn` VARCHAR(9), IN `cel` VARCHAR(10), IN `cla` VARCHAR(100), IN `idem` INT, IN `tp` INT)  NO SQL
UPDATE cliente SET nomcli=nom,apecli=ape,
corcli=co,nomdepacli=nomde,
nomprovcli=nompro,nomdiscli=nomdi,dnicli=dn,
celcli=cel,clavecli=cla,idempleado=idem,
tipo=tp
WHERE idcliente=idcli$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_departamento` (IN `id` INT, IN `nom` VARCHAR(100))  NO SQL
UPDATE departamento SET nomdepa=nom where iddepa=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_distritos` (IN `iddi` INT, IN `idpro` INT, IN `nom` INT)  NO SQL
UPDATE  distritos SET idprov=idpro,nomdis=nom WHERE iddis=iddi$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_empleados` (IN `idem` INT, IN `nom` VARCHAR(100), IN `ape` VARCHAR(100), IN `co` VARCHAR(100), IN `nomde` VARCHAR(100), IN `nompro` VARCHAR(100), IN `nomdi` VARCHAR(100), IN `dn` VARCHAR(9), IN `cel` VARCHAR(10), IN `cla` VARCHAR(100), IN `tp` INT)  NO SQL
UPDATE empleados SET nombre=nom,apellido=ape,
correo=co,nomdepa=nomde,
nomprov=nompro,nomdis=nomdi,dni=dn,
celular=cel,clave=cla,
tipo=tp
WHERE idempleado=idem$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_matricula` (IN `idmatri` INT, IN `idcur` INT, IN `idcli` INT, IN `fecha_ini` DATE, IN `fecha_ter` DATE)  NO SQL
UPDATE matricula SET idcurso=idcur,idcliente=idcli,fecha_inicio=fecha_ini,fecha_termino=fecha_ter where idmatricula=idmatri$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_profesor` (IN `idpro` INT, IN `nom` VARCHAR(100), IN `ape` VARCHAR(100), IN `co` VARCHAR(100), IN `nomde` VARCHAR(100), IN `nompro` VARCHAR(100), IN `nomdi` VARCHAR(100), IN `dn` VARCHAR(9), IN `cel` VARCHAR(10), IN `cla` VARCHAR(100), IN `idem` INT, IN `tp` INT)  NO SQL
UPDATE profesor SET nompro=nom,apepro=ape,
corpro=co,nomdepapro=nomde,
nomprovpro=nompro,nomdispro=nomdi,dnipro=dn,
celpro=cel,clavepro=cla,idempleado=idem,
tipo=tp
WHERE idprofesor=idpro$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_provincia` (IN `idpro` INT, IN `nom` VARCHAR(100), IN `idde` INT)  NO SQL
UPDATE  provincia SET nomprov=nom,iddepa=idde WHERE idprov=idpro$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_actualizar_turno` (IN `idtur` INT, IN `hr_i` TIME, IN `hr_f` TIME)  NO SQL
UPDATE turno SET hora_inicio=hr_i,hora_final=hr_f WHERE idturno=idtur$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cliente_id` (`id` INT)  BEGIN
SELECT idcliente,nomcli,apecli,corcli,dnicli,celcli,clavecli from cliente WHERE idcliente=id;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cliente_select` ()  BEGIN
SELECT idcliente,nomcli,apecli,corcli,dnicli,celcli,idempleado,tipo from cliente;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_docente_id` (IN `id` INT)  BEGIN
SELECT idprofesor,nompro,apepro,corpro,dnipro,celpro,clavepro from profesor WHERE idprofesor=id;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_docente_select` ()  BEGIN
SELECT idprofesor, nompro,apepro,corpro,dnipro,celpro,idempleado,tipo from profesor;
end$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_cargo` (IN `id` INT)  NO SQL
DELETE  FROM cargo WHERE idcargo=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_cliente` (IN `id` INT)  NO SQL
DELETE  FROM cliente WHERE idcliente=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_curso` (IN `id` INT)  NO SQL
BEGIN
DELETE  from curso WHERE idcurso=id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_departamento` (IN `id` INT)  NO SQL
DELETE  FROM departamento WHERE iddepa=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_distrito` (IN `id` INT)  NO SQL
DELETE FROM distritos WHERE iddis=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_empleados` (IN `id` INT)  NO SQL
DELETE FROM empleados  WHERE idempleado=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_matricula` (IN `id` INT)  NO SQL
DELETE FROM matricula where idmatricula=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_profesor` (IN `id` INT)  NO SQL
DELETE FROM profesor where idprofesor=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_provincia` (IN `id` INT)  NO SQL
DELETE FROM provincia WHERE idprov=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_eliminar_turno` (IN `id` INT)  NO SQL
DELETE FROM turno WHERE idturno=id$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_cargo` (IN `idcargo` INT, IN `nombre` VARCHAR(100))  NO SQL
INSERT INTO cargo (nombre) VALUES (nombre)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_cliente` (IN `idcliente` INT, IN `nom` VARCHAR(100), IN `ape` VARCHAR(100), IN `co` VARCHAR(100), IN `nomde` VARCHAR(100), IN `nompro` VARCHAR(100), IN `nomdi` VARCHAR(100), IN `dn` VARCHAR(9), IN `cel` VARCHAR(10), IN `cla` VARCHAR(255), IN `idem` INT, IN `tp` INT)  NO SQL
INSERT INTO cliente (nomcli,apecli,
corcli,nomdepacli,nomprovcli,nomdiscli,dnicli,celcli,
clavecli,idempleado,tipo) VALUES
(nom,ape,co,nomde,nompro,nomdi,dn,
cel,cla,idem,tp)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_departamento` (IN `iddepa` INT, IN `nom` VARCHAR(100))  NO SQL
INSERT INTO departamento (nomdepa) VALUES (nom)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_distrito` (IN `iddi` INT, IN `idpro` INT, IN `nom` VARCHAR(100))  NO SQL
INSERT INTO distritos  (idprov,nomdis) VALUES (idpro,nom)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_empleados` (IN `idempleado` INT, IN `nom` VARCHAR(100), IN `ape` VARCHAR(100), IN `co` VARCHAR(100), IN `nomde` VARCHAR(100), IN `nompro` VARCHAR(100), IN `nomdi` VARCHAR(100), IN `dn` VARCHAR(9), IN `cel` VARCHAR(10), IN `cla` VARCHAR(100), IN `tp` INT)  NO SQL
INSERT INTO empleados (nombre,apellido,
correo,nomdepa,nomprov,nomdis,dni,celular,
clave,tipo) VALUES
(nom,ape,co,nomde,nompro,nomdi,dn,
cel,cla,tp)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_matricula` (IN `idcur` INT, IN `idcli` INT, IN `fecha_ini` DATE, IN `fecha_ter` DATE)  NO SQL
INSERT INTO matricula (idcurso,idcliente,fecha_inicio,fecha_termino)
VALUES (idcur,idcli,fecha_ini,fecha_ter)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_profesor` (IN `idpro` INT, IN `nom` VARCHAR(100), IN `ape` VARCHAR(100), IN `co` VARCHAR(100), IN `nomde` VARCHAR(100), IN `nompro` VARCHAR(100), IN `nomdi` VARCHAR(100), IN `dn` VARCHAR(9), IN `cel` VARCHAR(10), IN `cla` VARCHAR(100), IN `idem` INT, IN `tp` INT)  NO SQL
INSERT INTO profesor (nompro,apepro,
corpro,nomdepapro,nomprovpro,nomdispro,dnipro,celpro,
clavepro,idempleado,tipo) VALUES
(nom,ape,co,nomde,nompro,nomdi,dn,
cel,cla,idem,tp)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_provincia` ()  NO SQL
INSERT INTO	 provincia (nomprov,iddepa) VALUES  (nom,idde)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_insertar_turno` (IN `idtur` INT, IN `hr_i` TIME, IN `hr_f` TIME)  NO SQL
INSERT into turno (hora_inicio,hora_final) VALUES (hr_i,hr_f)$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_registrar_select` ()  SELECT idempleado,nombre,apellido,correo,dni,celular,tipo from empleados$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_curso_id` (IN `id` INT)  NO SQL
BEGIN
SELECT idcurso,nomcurso,descripcion FROM curso WHERE  idcurso=id;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_select_id` (IN `id` INT)  SELECT idempleado,nombre,apellido,correo,dni,celular,clave from empleados  where idempleado=id$$
...
CREATE TABLE `cliente` (
  ...
  `nomdepacli` varchar(50) NOT NULL,
  `nomprovcli` varchar(50) NOT NULL,
  `nomdiscli` varchar(50) NOT NULL,
  `dnicli` varchar(20) NOT NULL,
  `celcli` varchar(20) NOT NULL,
  `clavecli` varchar(100) NOT NULL,
...
INSERT INTO `cliente` (`idcliente`, `nomcli`, `apecli`, `corcli`, `nomdepacli`, `nomprovcli`, `nomdiscli`, `dnicli`, `celcli`, `clavecli`, `idempleado`, `tipo`) VALUES
(1, 'carla', 'flores', 'carla@gmail.com', '3', '30', '278', '454545545', '964523552', '123', 2, 2),
(2, 'danielaaaaaaaaaaa', 'zaragoza', 'daniell@gmail.com', 'Lima', 'Lima', 'Rimac', '97859561', '788888887', '123', 2, 2),
(3, 'Juan', 'Chese', 'Juan@gmail.com', 'Lima', 'Lima', 'San Juan de Lurigancho', '40650124', '9932165487', '123', 2, 2),
(4, 'Maria', 'Sefue', 'Maria@gmail.com', 'Lima', 'Lima', 'Agustino', '60154658', '996548721', '123', 2, 2),
(6, 'osvaldo', 'lorem', 'lorem@gmail.com', '1', '2', '23', '45455447', '956321561', '$2y$12$PmLzDMx9JGHenXv2JdscNegJFx7Z6tWbrgfGs7g77A7nSueX/LKsm', 2, 2);
...
INSERT INTO `curso` (`idcurso`, `nomcurso`, `descripcion`) VALUES
(1, 'curso 1', 'nada'),
(2, 'curso 2', 'nada'),
(3, 'curso 3', 'nada'),
(4, 'curso 4', 'nada'),
...
INSERT INTO `departamento` (`iddepa`, `nomdepa`) VALUES
(1, 'AMAZONAS'),
(2, 'ANCASH'),
(3, 'APURIMAC'),
(4, 'AREQUIPA'),
...
INSERT INTO `distritos` (`iddis`, `nomdis`, `idprov`) VALUES
(1, 'CHACHAPOYAS', 1),
(2, 'ASUNCION', 1),
(3, 'BALSAS', 1),
(4, 'CHETO', 1),
...
INSERT INTO `empleados` (`idempleado`, `nombre`, `apellido`, `correo`, `nomdepa`, `nomprov`, `nomdis`, `dni`, `celular`, `clave`, `tipo`) VALUES
(1, 'andres', 'ttito', 'andres@gmail.com', '3', '30', '278', '75110109', '934909835', '123', 1),
(2, 'david', 'chavez Ttito', 'david@gmail.com', '3', '30', '278', '75110109', '934909835', '123', 1),
(16, 'jorge', 'chavez', 'jorge@gmail.com', '3', '29', '271', '10581610', '969989564', '$2y$12$nP/kf7b5nRMXN934tSstluUl5dQY/MUBif8mP2HBatFs0jM8a/Gla', 1);
...
INSERT INTO `matricula` (`idmatricula`, `idcurso`, `idcliente`, `fecha_inicio`, `fecha_termino`) VALUES
(1, 3, 3, '0000-00-00', '0000-00-00'),
(2, 3, 3, '0000-00-00', '0000-00-00'),
(3, 2, 2, '0000-00-00', '0000-00-00'),
(4, 2, 2, '0000-00-00', '0000-00-00'),
(5, 3, 3, '0000-00-00', '0000-00-00'),
(6, 3, 3, '0000-00-00', '0000-00-00'),
(7, 3, 3, '0000-00-00', '0000-00-00'),
(8, 3, 3, '0000-00-00', '0000-00-00'),
(9, 3, 3, '0000-00-00', '0000-00-00'),
(10, 3, 3, '0000-00-00', '0000-00-00'),
(11, 2, 2, '0000-00-00', '0000-00-00'),
(12, 3, 3, '0000-00-00', '0000-00-00'),
(13, 2, 2, '0000-00-00', '0000-00-00'),
(14, 2, 2, '0000-00-00', '0000-00-00'),
(15, 2, 2, '0000-00-00', '0000-00-00'),
(16, 2, 2, '0000-00-00', '0000-00-00'),
(17, 3, 3, '0000-00-00', '0000-00-00'),
(18, 2, 2, '0000-00-00', '0000-00-00'),
(19, 3, 3, '0000-00-00', '0000-00-00'),
(20, 2, 2, '0000-00-00', '0000-00-00'),
(21, 3, 3, '0000-00-00', '0000-00-00'),
(22, 1, 1, '0000-00-00', '0000-00-00'),
(23, 1, 1, '0000-00-00', '0000-00-00'),
(24, 4, 4, '0000-00-00', '0000-00-00');
...
CREATE TABLE `profesor` (
  ...
  `nomdepapro` varchar(50) NOT NULL,
  `nomprovpro` varchar(50) NOT NULL,
  `nomdispro` varchar(50) NOT NULL,
  `dnipro` varchar(20) NOT NULL,
  `celpro` varchar(20) NOT NULL,
  `clavepro` varchar(100) NOT NULL,
  `idempleado` int(11) NOT NULL,
  `tipo` int(11) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
...
INSERT INTO `profesor` (`idprofesor`, `nompro`, `apepro`, `corpro`, `nomdepapro`, `nomprovpro`, `nomdispro`, `dnipro`, `celpro`, `clavepro`, `idempleado`, `tipo`) VALUES
(1, 'ian', 'mendez', 'ian@gmail.com', '3', '30', '278', '75100239', '964523552', '123', 2, 3),
(5, 'agie', 'Floresxx', 'angie@gmail.com', '3', '30', '278', '45455447', '956321561', '$2y$12$Q5cFa8aTl.tLy2PbDlpMFOFO0y5S/hH56pqxlGIWXtRKqr4j3ZvIm', 2, 3);
...
INSERT INTO `provincia` (`idprov`, `nomprov`, `iddepa`) VALUES
(1, 'CHACHAPOYAS', 1),
(2, 'BAGUA', 1),
(3, 'BONGARA', 1),
(4, 'CONDORCANQUI', 1),
...
INSERT INTO `turno` (`idturno`, `hora_inicio`, `hora_final`) VALUES
(1, '15:08:00', '03:09:06');
...
ALTER TABLE `cliente`
  ...
  ADD KEY `iddepartamento` (`nomdepacli`,`nomprovcli`,`nomdiscli`,`idempleado`) USING BTREE,
  ADD KEY `iddistrito` (`nomdiscli`),
  ADD KEY `idprovincia` (`nomprovcli`);
...
ALTER TABLE `curso_profesor`
  ...
  ADD KEY `fk_curso_curpro` (`idcurso`),
  ADD KEY `fk_profe_curpro` (`idprofesor`);
...
ALTER TABLE `empleados`
  ...
  ADD KEY `iddepartamento` (`nomdepa`,`nomprov`,`nomdis`),
  ADD KEY `iddistrito` (`nomdis`),
  ADD KEY `idprovincia` (`nomprov`);
...
ALTER TABLE `matricula`
  ...
  ADD KEY `idcurso` (`idcurso`,`idcliente`),
  ADD KEY `idcurso_2` (`idcurso`,`idcliente`),
...
ALTER TABLE `profesor`
  ...
  ADD KEY `idempleado` (`idempleado`),
  ADD KEY `idprovincia` (`nomprovpro`),
  ADD KEY `iddistrito` (`nomdispro`),
  ADD KEY `iddepartamento` (`nomdepapro`);
...
--
ALTER TABLE `curso_profesor`
  MODIFY `id_curso_profesor` int(11) NOT NULL AUTO_INCREMENT;
...
--
ALTER TABLE `distritos`
  MODIFY `iddis` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1832;
...
--
ALTER TABLE `cliente`
  ADD CONSTRAINT `fk_cli_emple` FOREIGN KEY (`idempleado`) REFERENCES `empleados` (`idempleado`);

--
-- Filtros para la tabla `curso_profesor`
--
ALTER TABLE `curso_profesor`
  ADD CONSTRAINT `fk_curso_curpro` FOREIGN KEY (`idcurso`) REFERENCES `curso` (`idcurso`),
  ADD CONSTRAINT `fk_profe_curpro` FOREIGN KEY (`idprofesor`) REFERENCES `profesor` (`idprofesor`);

--
-- Filtros para la tabla `distritos`
--
ALTER TABLE `distritos`
  ADD CONSTRAINT `fk_prov_dis` FOREIGN KEY (`idprov`) REFERENCES `provincia` (`idprov`);

--
-- Filtros para la tabla `matricula`
--
ALTER TABLE `matricula`
  ADD CONSTRAINT `fk_matri_cli` FOREIGN KEY (`idcliente`) REFERENCES `cliente` (`idcliente`),
  ADD CONSTRAINT `fk_matri_cur` FOREIGN KEY (`idcurso`) REFERENCES `curso` (`idcurso`);

--
-- Filtros para la tabla `profesor`
--
ALTER TABLE `profesor`
  ADD CONSTRAINT `fk_prof_emple` FOREIGN KEY (`idempleado`) REFERENCES `empleados` (`idempleado`);

--
-- Filtros para la tabla `provincia`
--
ALTER TABLE `provincia`
  ADD CONSTRAINT `fk_depar_prov` FOREIGN KEY (`iddepa`) REFERENCES `departamento` (`iddepa`);
COMMIT;
